...
from src.procedure_handler import ProcedureHandler


class TestProcedureHandler():
    ...
    handler = ProcedureHandler(state_file=config.get('state-file'))
    ...
    def test_procedure_stop(self):
        mock_state = self.handler.update_state_record(0, 'started')
        stop = self.handler.stop()
        assert mock_state
        assert stop

    def test_procedure_cont(self):
        mock_sketch = self.update_sketch_file(self.sketch_file)
        mock_state = self.handler.update_state_record(0, 'paused')
        mock_state = self.handler.update_state_record(1, self.config['state-file'])
        mock_state = self.handler.update_state_record(2, 'Stage 1')
        mock_state = self.handler.update_state_record(3, 'Test Action 1')
        load = self.handler.load(self.sketch_file)
        resume = self.handler.cont()
        assert load
        assert mock_state
        assert resume

    def test_procedure_pause(self):
        mock_state = self.handler.update_state_record(0, 'started')
        pause = self.handler.pause()
        assert mock_state
        assert pause

    def test_procedure_setup(self):
        setup = self.handler.setup()
        assert setup

    def test_procedure_stage_handler(self):
        mock_stages = self.generate_mock_stages_dict()
        run_no_skip = self.handler.process_sketch_stages(mock_stages)
        print('(Run(No Skip) {}) {}'.format(run_no_skip, mock_stages))
        if run_no_skip != None:
            assert run_no_skip
        run_skip = self.handler.process_sketch_stages(
            mock_stages, skip_to=('Stage 1', 'Test Action 2')
        )
